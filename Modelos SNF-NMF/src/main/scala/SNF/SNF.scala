...
class SNF (val exp:DenseMatrix[Double],val methy:DenseMatrix[Double],val mirna:DenseMatrix[Double]){
 ...
 /* private def normalizar(matriz: DenseMatrix[Double]): DenseMatrix[Double] = {
    ...
    break()
    for (ele <-matriz.activeIterator)print(ele)
    return matriz
  }*/
  ...
  private def calcularDistancia(matriz: DenseMatrix[Double]): DenseMatrix[Double] = {
    ...
    for (i <- 0 to matriz.rows - 1) {
      for (j <- 0 to matriz.rows - 1) {
        var aux: Double = 0
        for (k <- 0 to matriz.cols - 1) {
  ...
  private def calcularPromedioDistancia(p: DenseMatrix[Double], porcentaje: Double): DenseMatrix[Double] = {
    ...
    var vecinos: Integer = calcularVecinos(p.rows, porcentaje)


    for (i <- 0 to p.rows - 1) {
      var fila: Array[Double] = new Array[Double](p.cols)
      for (j <- 0 to p.cols - 1) {
        fila(j) = p(i, j)
      }
  ...
  private def calcularKernelCompleto(matrizKernel: DenseMatrix[Double]): DenseMatrix[Double] = {
    ...
    for (i <- 0 to kernelCompleto.rows - 1) {
      for (j <- 0 to kernelCompleto.cols - 1) {
        if (i != j) {
  ...
  private def calcularKernelDispersa(matrizKernel: DenseMatrix[Double], porcentaje: Double): DenseMatrix[Double] = {
    ...
    for (i <- 0 to matrizKernel.rows - 1) {
      var fila: Array[Double] = new Array[Double](matrizKernel.cols)
      for (j <- 0 to matrizKernel.cols - 1) {
        fila(j) = matrizKernel(i, j)
      }
      val ordFila = fila.sorted.dropRight(fila.length - vecinos-1)
      var sumatoria: Double = 0
      for (i <- 1 to ordFila.length-1 ) {
        sumatoria += ordFila(i)
      }
      if (sumatoria == 0){
  ...
  def aplicarSNF(m:Double,porcentaje:Double,iteraciones:Integer): (DenseMatrix[Double],DenseMatrix[Double],DenseMatrix[Double],DenseMatrix[Double]) = {
    ...
    //for (ele <- mirnaKernelCompleto.activeIterator)print(ele)
    ...
    return (matrizEstatusPromedio,expMatrizEstatus,methyMatrizEstatus,mirnaMatrizEstatus)
  }
}
