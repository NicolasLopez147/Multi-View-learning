...
class NMFTest extends AnyFunSuite {
  ...
  implicit val rand: RandBasis = RandBasis.withSeed(1)

  //X es un arreglo de 3 matrices , que se usarán para probar el método jnmf
  val X: Array[DenseMatrix[Double]] = Array(
    DenseMatrix.rand[Double](70, 70),
    DenseMatrix.rand[Double](70, 70),
    DenseMatrix.rand[Double](70, 70)
  )
...
y el producto de W y cada una de las matrices H. Iteramos a través del arreglo X
 ...
 es el producto de W y la matriz H para ese elemento de X. Finalmente,
  calculamos la media de estos valores para obtener el porcentaje de error.*/
