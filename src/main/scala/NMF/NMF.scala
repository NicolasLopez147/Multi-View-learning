...
object Trainer {
  ...
  def jnmf(
      xss: Array[DenseMatrix[Double]],
      r: Int = 2,
      remplazo: Double = 0.00000000001,
      n: Int = 50000,
      eps: Double = 0.001,
      epsEval: Int = 1
  ): JNMFModel = {
    ...
    val (_, w, hs, _) = (1 to n).foldOrStop((xs, w0, hs0, cost0)) {
      case ((xs, w, hs, cost), i) =>
          ...
          nhs
            ...
            .foldLeft(
              (
                ...
                DenseMatrix.zeros[Double](w.cols, w.cols)
              )
            ) { case ((num, den), (h, x)) =>
              (num :+= x * h.t, den :+= h * h.t)
            }
...
case class JNMFModel(w: DenseMatrix[Double], hs: Array[DenseMatrix[Double]]) {
  ...
  def clustering(): (DenseMatrix[Double], Seq[Int]) = {
    val labels = (0 until w.rows)
      .foldLeft(List[Int]())((list_acc, row) => {
  ...
  def cost(xs: Array[DenseMatrix[Double]]): Double = {
    xs.zip(hs).foldLeft(0d) { case (cost, (x, h)) =>
      cost + sum(pow(x - w * h, 2))
    }
  }
}
